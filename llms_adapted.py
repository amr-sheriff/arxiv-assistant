...
from helpers_langchain import get_buffer_string_adapted  # adapted function

logger = logging.getLogger(__name__)


# Define the new subclass
class ChatPromptValue(ChatPromptValue):
    """Custom chat prompt value with overridden to_string method."""

    def to_string(self) -> str:
        """Return prompt as string with custom behavior."""
        return get_buffer_string_adapted(self.messages)
...
class BaseLLM(BaseLanguageModel[str], ABC):
    ...
    def _convert_input(self, input: LanguageModelInput) -> PromptValue:
        if isinstance(input, PromptValue):
            print("input is PromptValue") #####
            return input
        elif isinstance(input, str):
            print("input is str") ####
            return StringPromptValue(text=input)
        elif isinstance(input, Sequence):
            print("input is Sequence") #####
    ...
    async def abatch(
        ...
        if max_concurrency is None:
            try:
                print('abatch in base llm with max concurrency None')
                llm_result = await self.agenerate_prompt(
                    [self._convert_input(input) for input in inputs],
                    callbacks=[c.get("callbacks") for c in config],
                    tags=[c.get("tags") for c in config],
    ...
    def generate_prompt(
        ...
        print("prompt_strings_generate_prompt: ", prompt_strings) ####
    ...
    async def agenerate_prompt(
        ...
        # prompt_strings = [p.to_string() for p in prompts]  # ####
        prompt_strings = [p.to_messages() for p in prompts][0]  # ####
        prompt_strings = [ChatPromptValue(messages=prompt_strings).to_string()]  # ####
        print("prompt_strings_agenerate_prompt: ", prompt_strings)  # ####
